...
(function () {
    ...
    function OfficialDocDetailCheckerModalCtrl($scope,
        ...
        $scope.deleteDocItem = function (item, docData) {
            $scope.checkText = "是否修改：" + docData.archiveNumber;
            $scope.docData = docData;
            ngDialog.open({
                template: 'app/pages/officialDoc/listOfficialDoc/dialog/modifiedOfficialDocReviewSend_Modal.html',
        ...
        $scope.updateOfficialDocToServer_Modified = function(docData) {
        ...
        docTypeSelected = $filter('filter')(vm.docOptions, {
            option: $scope.docData.docType,
        });

        console.log(docTypeSelected);
        vm.docType = docTypeSelected[0];
